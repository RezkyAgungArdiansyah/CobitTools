    ...
    foreach($form_data as $row){
        ...
        if($slug == 'DF3'){
            foreach(['impact','likelihood'] as $col){
                $df_imp::where('id_'.$lower_slug,$i+1)->update([$col => $row[$j]]);
                $j++;}
            $df_imp::where('id_'.$lower_slug,$i+1)->update(['risk_rating' => $row[0]*$row[1]]);
            $df_imp::where('id_'.$lower_slug,$i+1)->update(['baseline' => $row[3]]);
        }
        elseif($slug != 'DF2' ){
            ...
            foreach(['importance','baseline'] as $col){
                $df_imp::where('id_'.$lower_slug,$arr[$i])->update([$col => $row[$j]]);
                $j++;}
            }
    $i++;
    }
